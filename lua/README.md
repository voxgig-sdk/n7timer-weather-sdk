...
The Lua SDK for the N7timerWeather API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Apipl(nil):list(nil, nil)
...
local result, err = client:N7timerWeather(nil):load(
  { id = "test01" }, nil
)
