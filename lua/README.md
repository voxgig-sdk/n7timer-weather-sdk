# N7timerWeather Lua SDK



The Lua SDK for the N7timerWeather API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Apipl()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
Create an instance: `local apipl = client:Apipl(nil)`
...
Create an instance: `local graphical_api = client:GraphicalApi(nil)`
...
```lua
local graphical_api, err = client:GraphicalApi():load({ lat = 1, lon = 1 })
```
...
```
lua/
├── n7timer-weather_sdk.lua    -- Main SDK module
...
The main module (`n7timer-weather_sdk`) exports the SDK constructor
