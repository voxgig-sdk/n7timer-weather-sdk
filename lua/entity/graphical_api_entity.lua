...
local GraphicalApiEntity = {}
GraphicalApiEntity.__index = GraphicalApiEntity


function GraphicalApiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GraphicalApiEntity)
  self._name = "graphical_api"
  self._client = client
...
function GraphicalApiEntity:get_name()
  return self._name
end


function GraphicalApiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GraphicalApiEntity.new(self._client, opts)
end
...
function GraphicalApiEntity:mark_deleted()
  self._deleted = true
end


function GraphicalApiEntity:deleted()
  return true == self._deleted
end


function GraphicalApiEntity:data_set(args)
  if args ~= nil then
...
function GraphicalApiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GraphicalApiEntity:match_set(args)
  if args ~= nil then
...
function GraphicalApiEntity:match_get()
...
function GraphicalApiEntity:stream(action, args, callopts)
...
---@param reqmatch GraphicalApiLoadMatch
---@param ctrl? table
---@return GraphicalApi
---@return string? err
function GraphicalApiEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GraphicalApiEntity:_run_op(ctx, post_done)
  local utility = self._utility
