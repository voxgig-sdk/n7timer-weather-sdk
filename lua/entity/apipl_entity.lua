...
local ApiplEntity = {}
ApiplEntity.__index = ApiplEntity


function ApiplEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ApiplEntity)
  self._name = "apipl"
  self._client = client
...
function ApiplEntity:get_name()
  return self._name
end


function ApiplEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ApiplEntity.new(self._client, opts)
end
...
function ApiplEntity:mark_deleted()
  self._deleted = true
end


function ApiplEntity:deleted()
  return true == self._deleted
end


function ApiplEntity:data_set(args)
  if args ~= nil then
...
function ApiplEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ApiplEntity:match_set(args)
  if args ~= nil then
...
function ApiplEntity:match_get()
...
function ApiplEntity:stream(action, args, callopts)
...
function ApiplEntity:_run_op(ctx, post_done)
  local utility = self._utility
