...
const items = await client.Apipl().list({ lat: 1, lon: 1, output: "example", product: "example" })
```
...
// apipls is an array of Apipl entities, populated with mock data
// — call apipls[0].data() for the record itself
console.log(apipls)
```
...
// List all apipls (returns ApiplEntity[] — .data() for the record)
...
$apipls = $client->Apipl()->list();
print_r(array_map(fn($item) => $item->data_get(), $apipls));
```
