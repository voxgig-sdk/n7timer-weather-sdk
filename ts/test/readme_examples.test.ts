...
//     (\`new N7timerWeatherSDK(...)\` / \`N7timerWeatherSDK.test(...)\` become
//     \`N7timerWeatherSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'N7timerWeatherSDK'
...
const TEST_SEED = {"entity":{"apipl":{"test01":{"id":"test01"}},"graphical_api":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new N7timerWeatherSDK(...)\` and any
// \`N7timerWeatherSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+N7timerWeatherSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/N7timerWeatherSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
