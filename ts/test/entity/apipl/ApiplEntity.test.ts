...
describe('ApiplEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'apipl.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set N_TIMER_WEATHER_TEST_APIPL_ENTID JSON to run live')
      return
    }
    ...
    const apipl_ref01_list = await apipl_ref01_ent.list(apipl_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['N_TIMER_WEATHER_TEST_APIPL_ENTID']
