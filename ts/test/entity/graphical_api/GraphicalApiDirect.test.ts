...
describe('GraphicalApiDirect', async () => {
  ...
  test('direct-load-graphical_api', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-graphical_api', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['N7TIMER_WEATHER_TEST_GRAPHICAL_API_ENTID']
