...
import (
	...
	omni "github.com/voxgig-sdk/n7timer-weather-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.N7timerWeatherError); is {
				out := map[string]any{
					"name":    "N7timerWeatherError",
...
func omniCtx(arg any, client *sdk.N7timerWeatherSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.N7timerWeatherSDK); is {
			client = live
