...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("N7TIMER_WEATHER_TEST_LIVE") == "TRUE" ||
		os.Getenv("N7TIMER_WEATHER_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("N7TIMER_WEATHER_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.N7timerWeatherSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.N7timerWeatherError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.N7timerWeatherSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.N7timerWeatherError{Msg: msg, Code: code}
}
