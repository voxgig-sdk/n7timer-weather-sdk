...
func TestApiplEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Apipl(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Apipl(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set N7TIMER_WEATHER_TEST_APIPL_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func apiplBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("N7TIMER_WEATHER_TEST_APIPL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["N7TIMER_WEATHER_TEST_APIPL_ENTID"])
