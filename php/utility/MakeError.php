...
class N7timerWeatherMakeError
{
    public static function call(?N7timerWeatherContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new N7timerWeatherContext([], null);
        }
        $op = $ctx->op ?? new N7timerWeatherOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new N7timerWeatherResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof N7timerWeatherError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new N7timerWeatherError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof N7timerWeatherError) {
