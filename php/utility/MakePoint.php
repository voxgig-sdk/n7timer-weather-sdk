...
// N7timerWeather SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class N7timerWeatherMakePoint
{
    public static function call(N7timerWeatherContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof N7timerWeatherError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = N7timerWeatherHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = N7timerWeatherHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
