...
// N7timerWeather SDK GraphicalApi entity
...
class GraphicalApiEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GraphicalApiEntity($this->_client, $opts);
    }

    /**
     * @param GraphicalApi|array $args GraphicalApi data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = N7timerWeatherHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GraphicalApi|array The current GraphicalApi data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GraphicalApi fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = N7timerWeatherHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GraphicalApi fields).
     */
    ...
    /**
     * Load a single GraphicalApi.
     *
     * @param GraphicalApiLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GraphicalApiLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GraphicalApi|array The loaded GraphicalApi as an assoc-array at the
     *   SDK boundary; throws N7timerWeatherError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = N7timerWeatherHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
