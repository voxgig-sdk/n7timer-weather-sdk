...
// N7timerWeather SDK feature test
//
...
class FtCtrl extends N7timerWeatherControl
{
...
class FtClient
{
    ...
    private N7timerWeatherUtility $utility;

    public function __construct(N7timerWeatherUtility $utility, array $options)
    {
    ...
    public function get_utility(): N7timerWeatherUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (N7timerWeatherContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public N7timerWeatherContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (N7timerWeatherContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = N7timerWeatherConfig::make_feature($name);
    ...
    private static function buildUrl(N7timerWeatherSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof N7timerWeatherError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new N7timerWeatherResponse($fetched);
        }
    ...
    private function fail(N7timerWeatherContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (N7timerWeatherContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
