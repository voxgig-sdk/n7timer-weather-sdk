# N7timerWeather PHP SDK



The PHP SDK for the N7timerWeather API — an entity-oriented client using PHP conventions.
...
try {
    ...
    foreach ($apipls as $record) {
        $item = $record->data_get();
        echo $item["dataseries"] . "\n";
    }
} catch (\Throwable $err) {
...
print_r(array_map(fn($item) => $item->data_get(), $apipl));
```
...
```
php/
├── n7timerweather_sdk.php          -- Main SDK class
...
The main class (`n7timerweather_sdk.php`) exports the SDK class
