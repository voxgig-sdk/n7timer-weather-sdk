# N7timerWeather Ruby SDK



The Ruby SDK for the N7timerWeather API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Apipl` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/n7timer-weather-sdk/releases](https://github.com/voxgig-sdk/n7timer-weather-sdk/releases)
...
```ruby
client = N7timerWeatherSDK.test(testopts, sdkopts)
```
...
raise a `N7timerWeatherError` (a `StandardError` subclass), so wrap
...
graphical_api = client.GraphicalApi.load({ "lat" => 1, "lon" => 1 })
```
...
```
rb/
├── N7timerWeather_sdk.rb       -- Main SDK module
...
The main module (`N7timerWeather_sdk`) exports the SDK class
...
# apipl.data_get now returns the apipl data from the last list
