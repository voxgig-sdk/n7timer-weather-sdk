...
module N7timerWeatherUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = N7timerWeatherResponse.new({})
    ...
    if fetch_err
      ...
      response = N7timerWeatherResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = N7timerWeatherResponse.new(fetched)
    else
