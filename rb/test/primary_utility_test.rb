...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = N7timerWeatherOmni.make_runner(TEST_JSON_FILE, N7timerWeatherSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    N7timerWeatherError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : N7timerWeatherOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = N7timerWeatherOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
