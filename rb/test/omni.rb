...
module N7timerWeatherOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if N7timerWeatherOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = N7timerWeatherOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| N7timerWeatherOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || N7timerWeatherOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = N7timerWeatherSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = N7timerWeatherError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { N7timerWeatherOmni.errify(err) }
