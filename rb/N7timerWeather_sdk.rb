...
require_relative 'N7timerWeather_types'
...
class N7timerWeatherSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = N7timerWeatherHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = N7timerWeatherHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, N7timerWeatherFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    N7timerWeatherUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = N7timerWeatherHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = N7timerWeatherHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = N7timerWeatherHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue N7timerWeatherError => err
    ...
    if fetched.is_a?(Hash)
      status = N7timerWeatherHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = N7timerWeatherError.new(
        "graphql_error", "N7timerWeatherSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = N7timerWeatherSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
