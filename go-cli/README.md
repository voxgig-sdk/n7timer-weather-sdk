...
boru-driven command-line client **and** interactive REPL for the N7timerWeather
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/n7timer-weather-cli)
make build

# 2. See usage (words, entities, env vars)
./n7timer-weather-cli --help

# 3. Provide credentials once, via the environment
export N7TIMER_WEATHER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./n7timer-weather-cli list apipl
./n7timer-weather-cli list graphical_api

# 5. Override the API base URL for a single call
N7TIMER_WEATHER_BASE=https://api.example.com ./n7timer-weather-cli list apipl

# 6. No arguments -> interactive REPL
./n7timer-weather-cli
n7timer-weather> list apipl
n7timer-weather> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/n7timer-weather-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/n7timer-weather-cli list apipl
   ```
...
Run with no arguments to open a REPL (prompt `n7timer-weather>`). Each line is
evaluated as its own boru expression:

```text
$ ./n7timer-weather-cli
n7timer-weather> list apipl
n7timer-weather> /help
n7timer-weather> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `apipl`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/n7timer-weather-cli`. |
...
(`./n7timer-weather-cli <expr>`) and interactively (the REPL), and expressions compose the
