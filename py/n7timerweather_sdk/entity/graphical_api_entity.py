...
from n7timerweather_sdk.n7timerweather_types import (
    GraphicalApi,
...
class GraphicalApiEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GraphicalApiEntity(self._client, opts)
    ...
    def data_get(self) -> GraphicalApi:
    ...
    def match_get(self) -> GraphicalApi:
    ...
    def load(self, reqmatch=None, ctrl=None) -> GraphicalApi:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.GraphicalApi().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
