...
class TestApiplDirect:

    def test_should_direct_list_apipl(self):
        setup = _apipl_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-apipl", "live" if setup["live"] else "unit")
        if _skip:
...
def _apipl_direct_setup(mockres):
