# N7timerWeather SDK test runner
...
class N7timerWeatherTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            N7timerWeatherTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = N7timerWeatherTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = N7timerWeatherTestRunner.getenv("N7TIMER_WEATHER_TEST_LIVE")
        override = N7timerWeatherTestRunner.getenv("N7TIMER_WEATHER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = N7timerWeatherTestRunner.getenv(key)
        ...
        explain = N7timerWeatherTestRunner.getenv("N7TIMER_WEATHER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if N7timerWeatherTestRunner._test_control is not None:
            return N7timerWeatherTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                N7timerWeatherTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            N7timerWeatherTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = N7timerWeatherTestRunner.load_test_control()
...
def load_env_local():
    N7timerWeatherTestRunner.load_env_local()


def env_override(m):
    return N7timerWeatherTestRunner.env_override(m)


def entity_data(v):
    return N7timerWeatherTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return N7timerWeatherTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return N7timerWeatherTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return N7timerWeatherTestRunner.load_test_control()


def live_client_options():
    return N7timerWeatherTestRunner.live_client_options()


def live_delay_ms():
    return N7timerWeatherTestRunner.live_delay_ms()
