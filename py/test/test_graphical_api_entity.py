    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set N_TIMERWEATHER_TEST_GRAPHICAL_API_ENTID JSON to run live")
        client = setup["client"]
        ...
        graphical_api_ref01_data_dt0_loaded, err = graphical_api_ref01_ent.load(graphical_api_ref01_match_dt0, None)
        assert err is None
...
def _graphical_api_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("N_TIMERWEATHER_TEST_GRAPHICAL_API_ENTID"))
