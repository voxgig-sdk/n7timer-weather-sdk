...
class TestApiplEntity:
    ...
    def test_should_stream(self):
        ...
        base = N7timerWeatherSDK.test(seed, None)
        seen = list(base.Apipl(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = N7timerWeatherSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Apipl(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set N7TIMER_WEATHER_TEST_APIPL_ENTID JSON to run live")
        client = setup["client"]
...
def _apipl_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("N7TIMER_WEATHER_TEST_APIPL_ENTID"))
