...
The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Apipl()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/n7timer-weather-sdk/releases)) or
...
```python
try:
    apipls = client.Apipl().list({"lat": 1, "lon": 1, "output": "example", "product": "example"})
    for apipl in apipls:
        print(apipl)
...
```
py/
├── n7timerweather_sdk.py         -- Main SDK module
