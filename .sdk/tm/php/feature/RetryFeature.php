...
// N7timerWeather SDK retry feature
...
class N7timerWeatherRetryFeature extends N7timerWeatherBaseFeature
{
    ...
    public function init(N7timerWeatherContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (N7timerWeatherContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(N7timerWeatherContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(N7timerWeatherContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
