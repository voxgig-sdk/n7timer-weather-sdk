...
//    only - a typed N7timerWeatherContext there would make every ctx
...
final class N7timerWeatherOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new N7timerWeatherContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new N7timerWeatherSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new N7timerWeatherResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new N7timerWeatherError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new N7timerWeatherResponse($clean['response']);
    ...
    public static function err_from_map($m): ?N7timerWeatherError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new N7timerWeatherError(is_string($code) ? $code : '', $msg);
    }
