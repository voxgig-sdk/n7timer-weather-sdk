...
[MCP](https://modelcontextprotocol.io) server exposing the N7timerWeather SDK as
two agent tools — `n7timer-weather_list` and `n7timer-weather_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/n7timer-weather-mcp)
make build

# 2. Provide credentials via the environment
export N7TIMER_WEATHER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user n7timer-weather \
  -- /absolute/path/to/n7timer-weather-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./n7timer-weather-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// n7timer-weather_list: first page of records
{ "entity": "apipl" }
{ "entity": "apipl", "query": { } }

// n7timer-weather_load: one record by id
{ "entity": "graphical_api", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/n7timer-weather-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user n7timer-weather \
     -- "$PWD"/dist/*/n7timer-weather-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `n7timer-weather_list` and `n7timer-weather_load` tools now appear
   in new sessions. Ask the agent to *"list apipl using n7timer-weather"*
   and it calls `n7timer-weather_list` with `{"entity":"apipl"}`.
...
```sh
export N7TIMER_WEATHER_APIKEY=sk_live_xxx            # API key
export N7TIMER_WEATHER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `n7timer-weather_list` tool
...
### Call the `n7timer-weather_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `n7timer-weather_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `n7timer-weather_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `N7TIMER_WEATHER_APIKEY` | API key sent with every request. |
| `N7TIMER_WEATHER_BASE` | Optional override of the API base URL. |
...
```sh
./n7timer-weather-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"n7timer-weather_load","arguments":{"entity":"graphical_api","query":{"id":1}}}}'
```
